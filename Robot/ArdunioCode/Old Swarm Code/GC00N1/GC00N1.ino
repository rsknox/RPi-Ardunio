...
uint8_t NODE_ADDRESS = 0;  /* Select node 1 for Ground Controller address = 00 */
/***********************************************************************/
/***********************************************************************/
...
int active_nodes [2] = {0, 1}; /* we are node 0 (Ground Controller); we talk only with the Flight Leader, node 1 */
...
//        }
//    }
int to = 1; /* we will always talk only to the FL, node 1 */
    ...
    if ( this_node > 00 || to == 00 ){                    // Normal nodes send a 'T' ping
      ...
      ok = send_T(to);  /* we will alway execute this branch as 'this_node' is alway 0 and 'to' is always 1 */
//        ok = send_N(to);
    }
...
//    add_node(header.from_node); /* I think we don't want to add anything to the node list, as it will be static */
}
/**
